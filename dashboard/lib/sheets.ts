...
import { addMonthsUTC, parseBool, parseChileanDate, parseNumber } from "./utils";
...
function expandirCuotas(movimientos: Movimiento[], warnings: string[]): Movimiento[] {
  ...
  for (const m of movimientos) {
    ...
    // "Cuota a pagar" llega del sheet en valor absoluto; montoCLP con signo.
...
export async function loadDashboardData(): Promise<DashboardData> {
  ...
  return {
    movimientos: expandirCuotas(movimientos, warnings),
    taxonomia,
    presupuesto,
    tipoCambio,
    deudasMaestro,
